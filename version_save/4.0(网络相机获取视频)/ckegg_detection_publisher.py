...
class ChickenFarmNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('onvif_ip', '172.20.10.5'),
                ('onvif_port', 80),
                ('onvif_username', 'admin'),
                ('onvif_password', '88888888'),
        ...
        self.onvif_ip = str(self.get_parameter('onvif_ip').value)
        self.onvif_port = int(self.get_parameter('onvif_port').value)
        self.onvif_username = str(self.get_parameter('onvif_username').value)
        self.onvif_password = str(self.get_parameter('onvif_password').value)
        self.onvif_profile_index = int(self.get_parameter('onvif_profile_index').value)
    ...
    def _inject_rtsp_credentials(self, rtsp_url: str, username: str, password: str) -> str:
        try:
            if not rtsp_url or not rtsp_url.startswith('rtsp://'):
                return rtsp_url
            if '@' in rtsp_url.split('rtsp://', 1)[1].split('/', 1)[0]:
                return rtsp_url
            if not username:
                return rtsp_url
            ...
            u = urllib.parse.urlsplit(rtsp_url)
    ...
    def _get_onvif_rtsp_url(self) -> str:
        if ONVIFCamera is None:
            raise RuntimeError('未安装 onvif 依赖库，无法使用 ONVIF 获取网络摄像机画面')
        ...
        for attempt in range(max_retries):
            try:
                ...
                sock.settimeout(5.0)
                result = sock.connect_ex((self.onvif_ip, self.onvif_port))
                sock.close()
                
                if result != 0:
                    raise ConnectionError(f"网络不可达: {self.onvif_ip}:{self.onvif_port}")
                
                self.get_logger().info(f"尝试连接 ONVIF 摄像机 (第{attempt+1}次): {self.onvif_ip}:{self.onvif_port}")
                ...
                # 设置连接超时
                cam.transport.operation_timeout = timeout
                cam.transport.connect_timeout = timeout
                
                media = cam.create_media_service()
                profiles = media.GetProfiles()
                if not profiles:
                    raise RuntimeError('ONVIF 未返回任何媒体 Profiles')

                idx = self.onvif_profile_index
                ...
                rtsp_url = getattr(stream_uri, 'Uri', None) or getattr(stream_uri, 'uri', None) or str(stream_uri)
                rtsp_url = self._inject_rtsp_credentials(rtsp_url, self.onvif_username, self.onvif_password)
                
                self.get_logger().info(f"成功获取 ONVIF RTSP URL: {rtsp_url}")
                return rtsp_url
                
            except (ConnectTimeoutError, ConnectionError, socket.timeout, socket.error) as e:
                self.get_logger().warning(f"ONVIF 连接失败 (第{attempt+1}次): {e}")
                if attempt == max_retries - 1:
                    # 最后一次尝试失败，提供备用方案
                    self._suggest_fallback_options()
                    raise RuntimeError(f'无法连接到 ONVIF 摄像机 {self.onvif_ip}:{self.onvif_port}，请检查网络连接和摄像机配置')
                time.sleep(2)  # 等待2秒后重试
                
            except Exception as e:
                self.get_logger().error(f"ONVIF 未知错误 (第{attempt+1}次): {e}")
                if attempt == max_retries - 1:
                    raise RuntimeError(f'ONVIF 协议错误: {e}')
                time.sleep(2)
    
    def _diagnose_network_connection(self):
        """诊断网络连接问题"""
        self.get_logger().info("=== 网络连接诊断 ===")
        
        # 1. 测试基本网络连通性
        import subprocess
        try:
            result = subprocess.run(['ping', '-c', '3', self.onvif_ip], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                self.get_logger().info(f"✅ 网络连通: {self.onvif_ip}")
            else:
                self.get_logger().error(f"❌ 网络不通: {self.onvif_ip}")
                self.get_logger().error(f"   {result.stderr}")
        except Exception as e:
            self.get_logger().error(f"❌ ping 测试失败: {e}")
        
        # 2. 测试端口连通性
        import socket
        for port in [80, 554, 8080, 8000]:  # 常见的摄像机端口
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(3.0)
                result = sock.connect_ex((self.onvif_ip, port))
                sock.close()
                if result == 0:
                    self.get_logger().info(f"✅ 端口 {port} 开放")
                else:
                    self.get_logger().warning(f"⚠️ 端口 {port} 不可达")
            except Exception as e:
                self.get_logger().warning(f"⚠️ 端口 {port} 测试失败: {e}")
        
        self.get_logger().info("===================")

    def _suggest_fallback_options(self):
        """提供备用连接方案建议"""
        self.get_logger().error("=== ONVIF 连接失败，可能的解决方案 ===")
        
        # 运行网络诊断
        self._diagnose_network_connection()
        
        self.get_logger().error("1. 检查摄像机配置:")
        self.get_logger().error("   - 确保 ONVIF 服务已启用")
        self.get_logger().error("   - 检查用户名密码是否正确")
        self.get_logger().error(f"   - 当前配置: {self.onvif_username}@{self.onvif_ip}:{self.onvif_port}")
        self.get_logger().error("2. 尝试不同的 RTSP URL 格式:")
        self.get_logger().error(f"   rtsp://{self.onvif_username}:{self.onvif_password}@{self.onvif_ip}:554/stream1")
        self.get_logger().error(f"   rtsp://{self.onvif_username}:{self.onvif_password}@{self.onvif_ip}:554/live")
        self.get_logger().error(f"   rtsp://{self.onvif_username}:{self.onvif_password}@{self.onvif_ip}:554/cam/realmonitor?channel=1&subtype=0")
        self.get_logger().error("3. 使用命令行参数测试:")
        self.get_logger().error(f"   ros2 run ckegg_detect ckegg_detection_publisher --ros-args -p source:='rtsp://{self.onvif_username}:{self.onvif_password}@{self.onvif_ip}:554/stream1'")
        self.get_logger().error("4. 使用本地摄像头测试:")
        self.get_logger().error("   ros2 run ckegg_detect ckegg_detection_publisher --ros-args -p source:='0'")
        self.get_logger().error("5. 使用视频文件测试:")
        self.get_logger().error("   ros2 run ckegg_detect ckegg_detection_publisher --ros-args -p source:='/path/to/video.mp4'")
    ...
    def _setup_input_source(self):
        ...
        src_lower = self.source.strip().lower() if self.source else ''
        use_onvif = (src_lower in ['', 'onvif'])
        is_rtsp_url = src_lower.startswith('rtsp://')
        ...
        if use_onvif:
            self.get_logger().info(f"🎥 使用 ONVIF 网络摄像机: {self.onvif_ip}:{self.onvif_port}")
            rtsp_url = "rtsp://172.20.10.5:554/stream1"
            self.get_logger().info(f"RTSP: {rtsp_url}")
            self._is_live_stream = True
            self.cap = cv2.VideoCapture(rtsp_url)
            if not self.cap.isOpened():
                raise RuntimeError('无法打开 ONVIF/RTSP 视频源')
            self.timer = self.create_timer(1.0 / 30.0, self.timer_callback)
            return

        if is_rtsp_url:
            self.get_logger().info(f"🎥 使用 RTSP 视频流: {self.source}")
            self._is_live_stream = True
            self.cap = cv2.VideoCapture(self.source)
            if not self.cap.isOpened():
                raise RuntimeError('无法打开 RTSP 视频源')
            self.timer = self.create_timer(1.0 / 30.0, self.timer_callback)
            return
    ...
    def timer_callback(self):
        ...
        if not ret:
            if getattr(self, '_is_live_stream', False):
                now = time.time()
                if now - self._last_timer_warn_ts > self._timer_warn_interval:
                    self._last_timer_warn_ts = now
                    self.get_logger().warning('⚠️ 无法读取视频帧，尝试重连...')
                try:
                    self.cap.release()
                except Exception:
                    pass
                ...
                try:
                    src_lower = self.source.strip().lower() if self.source else ''
                    if src_lower in ['', 'onvif']:
                        rtsp_url = "rtsp://172.20.10.5:554/stream1"
                        self.cap = cv2.VideoCapture(rtsp_url)
                    else:
                        self.cap = cv2.VideoCapture(self.source)
                except Exception as e:
                    self.get_logger().warning(f"重连失败: {e}")
                return
    ...
    def save_cage_data(self, c_id, data):
        if self._is_id_jump(c_id):
            self.get_logger().info(f"SKIP_UPLOAD due to ID jump: {c_id}")
            return
